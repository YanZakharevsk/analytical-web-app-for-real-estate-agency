package com.hoxsik.courseproject.real_estate_agency.exceptions;
...
public class RestControllerAdvice {
    ...
    public ResponseEntity<Response> handleAuthenticationException(AuthenticationException e) {
        if (e.getMessage().equals("Недействительный токен")) {

            String message = "Предоставлен недействительный токен";
            log.error(message);

            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new Response(false, HttpStatus.UNAUTHORIZED, message));
        } else {

            String message = "Не удалось аутентифицировать пользователя";
            log.error(message);
